...
def get_image_by_id(token_id, image_id):
    """获取所有的镜像信息"""
    headers = {"Content-type": "application/json", "X-Auth-Token": token_id, "Accept": "application/json"}
...
def allowed_file(filename):
    """用于镜像上传的文件名后缀是否合法"""
    ALLOWED_EXTENSIONS = set(['qcow2', 'img', 'ami', 'ari', 'aki', 'vhd', 'vhdx', 'vmdk', 'raw', 'vdi', 'iso'])  # 文件末尾名集合
...
def create_image(token_id, data, file_name):
    ...
    file_dir = os.path.join(basedir, 'upload', file_name)
    is_upload_ok = upload_image_info(token_id, image_id, file_dir)
    if is_upload_ok == 204:
        image_info = get_image_by_id(token_id, image_id)
    os.remove(file_dir)
    return image_info
